...
public final class ElementSerializer extends Object {
   ...
   /**
    * Writes XML for an <code>Element</code> to a character stream.
    *
    * @param out
    *    the character stream to write to, cannot be <code>null</code>.
    *
    * @param xmlout
    *    the XML encoder that can be used to write XML to the character
    *    stream, cannot be <code>null</code>.
    *
    * @param element
    *    the {@link Element} object to convert to XML, cannot be
    *    <code>null</code>.
    *
    * @throws NullPointerException
    *    if <code>out == null || xmlout == null || element == null</code>.
    *
    * @throws IOException
    *    if there is an I/O error.
    */
   public void output(Writer out, XMLEncoder xmlout, Element element)
   throws NullPointerException, IOException {
      ...
      if (namespaceURI != null) {
         ...
         // Write the start tag with prefix
         // NOTE: The prefix is guaranteed to be valid
         // TODO: Make sure local name is valid and does not need escaping
         out.write('<');
         out.write(qName);

         // Associate the namespace with the prefix in the result XML
         if (newNamespace) {
            xmlout.attribute(out, "xmlns:" + prefix, namespaceURI, '"', true);
         }
      ...
      while (names.hasNext()) {
         ...
         if (attrNamespaceURI != null) {
            ...
            // Write the attribute with prefix
            xmlout.attribute(out, prefix + ':' + attrLocalName, attrValue, '"', true);

            // Associate the namespace with the prefix in the result XML
            if (newNamespace) {
               xmlout.attribute(out, "xmlns:" + prefix, attrNamespaceURI, '"', true);
            }

         // Write an attribute without namespace
         } else {
            xmlout.attribute(out, attrLocalName, attrValue, '"', true);
         }
      }

      // End this opening tag
      out.write('>');
      ...
      for (int i = 0; i < count; i++) {
         Object o = content.get(i);
         output(out, xmlout, (Element) o);
      }

      // Output contained PCDATA
      if (element.getText() != null) {
         xmlout.text(out, element.getText(), true);
      }
